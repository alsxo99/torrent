...
*/
    ...
    for (int i = 0; i < num_torrents; i++)
    {
        ...
        {
            if (torrent->block_info[block_idx] == 0) // if문 내부의 block_idx 는 내가 없는 block의 index이다.
            {
                for (int peer_idx = 0; peer_idx < torrent->num_peers; peer_idx++)
                {
                    if (torrent->peer_block_info[peer_idx][block_idx] == 1) // if문 내부의 peer_idx는 내가 없는 block을 가지고 있는 peer의 index이다.
                    {
                        ...
                        // 위의 if문을 false로 통과했다면, peer에게 block을 요청할 것이다.
                        int result = -1;
                        while (result == -1)
                        {
                            // request_block_from_peer 로 해당 block을 peer에게 요청한다.
                            // 해당 peer에게서 내가 기다리고 있는 request num을 1 늘려준다.
                            // 성공하면 result에 0이, 실패하면 -1이 저장될 것이다.
                            result = request_block_from_peer(torrent->peer_ip[peer_idx], torrent->peer_port[peer_idx], torrent, block_idx);
                            torrent->peer_req_num[peer_idx] += 1;
                            // 성공했을 경우, peer_req_num 을 0으로 만들어준다.
                            if (result == 0)
                                torrent->peer_req_num[peer_idx] = 0;
                            // request가 실패했고, peer_req_num 이 PEER_EVICTION_REQ_NUM 보다 크면, remove_peer_from_torrent 해준다.
                            if (result == -1 && torrent->peer_req_num[peer_idx] > PEER_EVICTION_REQ_NUM)
                            {
